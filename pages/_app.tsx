...
function MyApp({ Component, pageProps }: AppProps) {
  ...
  const store_setPartialInitSuccess = useDataStore(
    (s) => s.setPartialInitSuccess
  );
  ...
  const handleInit = async () => {
    ...
    if (!store_partialInitSuccess) {
      ...
      //& get first page data
      await Promise.all(
        Object.entries(contractDataMap).map(
          async (ctr: any, ctr_idx: number) => {
            ...
            let REQ = await axios.get(
              `https://api.opensea.io/api/v1/assets?order_direction=asc&asset_contract_address=${CTR_ADDR}&limit=200&include_orders=false`,
              {
                headers: {
            ...
            REQ.data.assets.forEach((ass: any) => {
              tempAssetArray.unshift({
                ...ass,
                asset_contract: CTR_ADDR,
                permalink: "---",
                collection: "---",
              });
    ...
    // //& get next page data
    await Promise.all(
      Object.entries(contractDataMap).map(async (ctr: any, ctr_idx: number) => {
        const CTR_NAME = ctr[0];
        const CTR_ADDR = ctr[1];
        ...
        const getNextPage = async () => {
          REQ = await axios.get(
            `https://api.opensea.io/api/v1/assets?order_direction=desc&asset_contract_address=${CTR_ADDR}&limit=200&include_orders=false&cursor=${nextDataCursor}`,
            {
              headers: {
                "X-API-KEY": env.OPENSEA_API_KEY,
              },
    ...
    Object.entries(store_contractData).forEach((ctr: any, ctr_idx: number) => {
      ...
      CTR_DATA.assets.forEach((ast: any, ast_idx: number) => {
        if (arrayOfOwnedTokenIds.includes(ast.token_id)) {
          ...
          console.log(
            `CONNECT | Not owned token: ${ast.token_id} => ${arrayOfOwnedTokenIds}`
          );
        }
      });
    });
